...
source "virtualbox-iso" "jessie" {
  boot_command            = ["<esc><wait>", "auto <wait>", "url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/debian-8/preseed.cfg <wait>", "hostname=jessie <wait>", "domain=vagrantup.com <wait>", "frontend=noninteractive <wait>", "<enter><wait>"]
  boot_wait               = "5s"
  disk_size               = 20480
